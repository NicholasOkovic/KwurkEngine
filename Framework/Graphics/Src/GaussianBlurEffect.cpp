...
void GaussianBlurEffect::Inittialize()
{
...
{
	ASSERT(mSourceTexture != nullptr, "GAussianEffect : sourcetexture is null");
	GraphicsSystem* gs = GraphicsSystem::Get();
...
void GaussianBlurEffect::DebugUI()
{
	if (ImGui::CollapsingHeader("GaussianBlur", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::DragFloat("BlurSaturations", &mBlurSaturation, 0.001, 1.0f, 100.0f);
	}
}
