...
{
	ASSERT(sGraphicsSystem == nullptr, "Graphics System: is already initialized");
...
{
	ASSERT(sGraphicsSystem != nullptr, "Graphics system: was not initialized");
	return sGraphicsSystem.get();
}

GraphicsSystem::~GraphicsSystem()
{
	ASSERT(mD3DDevice == nullptr, "GraphicsSystem: Termiante must be called");
}
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
	{
		...
		ASSERT(SUCCEEDED(hr), "GraphicsSystem: failede to access swap cxhain");
	...
	ASSERT(SUCCEEDED(hr), "Graphic system: failed to get back buffer data");

	hr = mD3DDevice->CreateRenderTargetView(backBuffer, nullptr, &mRenderTargetView);
	SafeRelease(backBuffer);
	ASSERT(SUCCEEDED(hr), "Graphic system: failed to create render target");
	...
	ASSERT(SUCCEEDED(hr), "Graphic system: failed to create stencil buffer");
	...
	ASSERT(SUCCEEDED(hr), "Graphic system: failed to create stencil view");
...
uint32_t Graphics::GraphicsSystem::GetBackBufferWidth() const
{
	return mSwapChainDesc.BufferDesc.Width;
}

uint32_t Graphics::GraphicsSystem::GetBackBufferHeight() const
{
	return  mSwapChainDesc.BufferDesc.Height;
}

float Graphics::GraphicsSystem::GetBackBufferAspectRatio() const
{
	return static_cast<float>(GetBackBufferWidth()) / static_cast <float>(GetBackBufferHeight());
}

ID3D11Device* Graphics::GraphicsSystem::GetDevice()
{
	return mD3DDevice;
}

ID3D11DeviceContext* Graphics::GraphicsSystem::GetContext()
{
