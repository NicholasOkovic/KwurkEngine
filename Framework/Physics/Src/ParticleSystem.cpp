...
void ParticleSystem::DebugUI()
{
	if (ImGui::CollapsingHeader("ParticleSystem"))
	{
		...
		ImGui::DragFloat3("StartColorMin", &mInfo.startColor.min.r);
		ImGui::DragFloat3("StartColorMax", &mInfo.startColor.max.r);
		ImGui::DragFloat3("EndColorMin", &mInfo.endColor.min.r);
		ImGui::DragFloat3("EndColorMax", &mInfo.endColor.max.r);
	}
}
...
{
	int numParticles = mInfo.particlesPerEmit.GetRandonInc();
...
void ParticleSystem::Render(Graphics::ParticleSystemEffect& effect)
{
	...
	effect.SetTextureId(mInfo.textrueId);
	for (auto& particle : mParticles)
	{
		if (particle->IsActive())
		{
